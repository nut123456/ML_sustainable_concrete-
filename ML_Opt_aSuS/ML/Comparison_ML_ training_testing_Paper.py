# -*- coding: utf-8 -*-
"""
Created on Thu Apr 17 21:34:29 2025
...
fig, axes = plt.subplots(n_rows, n_cols, figsize=(5 * n_cols, 5.5 * n_rows), sharey=True)
axes = axes.flatten()
...
for idx, (name, model) in enumerate(models.items()):
    ...
    ax = axes[idx]
    ax.scatter(y_train, y_train_pred, color='blue', s=20, label='Train data')
    ax.scatter(y_test, y_test_pred, facecolors='none', edgecolors='red', s=20, label='Test data')

    lims = [min(y.min(), y_train_pred.min(), y_test_pred.min()), max(y.max(), y_train_pred.max(), y_test_pred.max())]
    ax.plot(lims, lims, 'k--', linewidth=1.2, label='1:1 Line')
    ax.plot(lims, [v*1.2 for v in lims], 'r--', linewidth=1)
    ax.plot(lims, [v*0.8 for v in lims], 'r--', linewidth=1)

    ax.set_title(name, fontsize=20, fontweight='bold')  # No italic
    ax.set_xlim(lims)
    ax.set_ylim(lims)
...
results_df.sort_values(by='Test RMSE', inplace=True)
print(results_df.to_string(index=False))

# วาดกราฟ
fig, axes = plt.subplots(2, 2, figsize=(12, 10))

axes[0, 0].bar(results_df['Model'], results_df['Test MSE'])
axes[0, 0].set_title('Test MSE')
axes[0, 1].bar(results_df['Model'], results_df['Test MAE'])
axes[0, 1].set_title('Test MAE')
axes[1, 0].bar(results_df['Model'], results_df['Test RMSE'])
axes[1, 0].set_title('Test RMSE')
axes[1, 1].bar(results_df['Model'], results_df['Test R2'])
axes[1, 1].set_title('Test R-squared')
